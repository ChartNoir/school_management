...
document
  .getElementById("createEnrollmentForm")
  ?.addEventListener("submit", function (e) {
    e.preventDefault();
...
function showPasswordPrompt() {
  if (window.userRole === "student" || window.userRole === "teacher") {
    const dialog = new bootstrap.Modal(
      document.getElementById("passwordPromptDialog")
    );
    dialog.show();
  } else {
...
function showResetTokenDialog(message, token) {
  const dialog = new bootstrap.Modal(
    document.getElementById("resetTokenDialog")
  );
...
function showAdminContactDialog(message) {
  const dialog = new bootstrap.Modal(
    document.getElementById("adminContactDialog")
  );
  document.getElementById("adminContactMessage").innerHTML =
    message + "<br><br>Contact an administrator:<br>" + window.adminEmails;
  dialog.show();
}
    ...
    .then((data) => {
      ...
      const displayFields =
        table === "users"
          ...
          : table === "class"
          ? fields.filter(
              (field) => !["created_at", "updated_at"].includes(field)
            )
          : fields;
      ...
      data.result.forEach((row, index) => {
        ...
        displayFields.forEach((field) => {
          ...
          } data-field="${field}" data-id="${row.id}">${
...
function saveEdit(table, id, rowIndex) {
  const row = document.querySelectorAll("#editRecordsOutput table tbody tr")[
    rowIndex
  ];
...
function loadEditFieldOptions() {
  ...
  if (!table) {
    fieldFilterMenu.innerHTML =
      '<li class="dropdown-item">Select a table first</li>';
    document.getElementById("editFieldFilterDropdown").textContent =
      "Choose Fields";
    return;
  }
  ...
  fetch("functions.php", {
    ...
    .then((data) => {
      if (data.result && Array.isArray(data.result)) {
        const visibleFields = data.result.filter(
        ...
        document.getElementById(
          "editFieldFilterDropdown"
        ).textContent = `Choose Fields (${visibleFields.length} available)`;

        fieldFilterMenu
          .querySelectorAll('input[name="edit_fields"]')
          .forEach((checkbox) => {
            checkbox.addEventListener("change", () => {
              const selectedCount = fieldFilterMenu.querySelectorAll(
                'input[name="edit_fields"]:checked'
              ).length;
              document.getElementById(
                "editFieldFilterDropdown"
              ).textContent = `Choose Fields (${selectedCount} selected, ${visibleFields.length} available)`;
            });
          });
      } else {
        fieldFilterMenu.innerHTML = `<li class="dropdown-item">Error loading fields: ${
...
function loadEditRecords(page = 1) {
  ...
  const output = document.getElementById("editRecordsOutput");
  const checkboxes = document.querySelectorAll(
    '#editFieldFilterMenu input[type="checkbox"]'
  );
  ...
  fetch("functions.php", {
    ...
    .then((data) => {
      ...
      paginatedData.forEach((row, index) => {
        ...
        html += `<td>
                <button class="btn btn-success btn-sm" onclick="saveEdit('${table}', ${
          row.id
        }, ${startIndex + index})">Save</button>
                <button class="btn btn-danger btn-sm" onclick="deleteRecord('${table}', ${
          row.id
        })">Delete</button>
            </td>`;
        html += "</tr>";
      ...
      let paginationHtml =
        '<nav aria-label="Edit records navigation"><ul class="pagination justify-content-center">';
      paginationHtml +=
        '<li class="page-item ' +
        (currentPage === 1 ? "disabled" : "") +
        '"><a class="page-link" href="#" onclick="loadEditRecords(' +
        (currentPage - 1) +
...
function saveEdit(table, id, rowIndex) {
  ...
  fetch("functions.php", {
    ...
    .then((data) => {
      ...
      if (data.result === true) {
        const currentPage =
          document.querySelector(".pagination .active .page-link")
            ?.textContent || 1;
...
document
  .getElementById("editFieldFilterMenu")
...
document
  .querySelector("#passwordPromptDialog .btn-close")
  ?.addEventListener("click", closePasswordPrompt);
document
...
document
  .querySelector("#resetTokenDialog .btn-close")
  ?.addEventListener("click", closeDialog);
document
  .getElementById("copyTokenBtn")
  ?.addEventListener("click", copyTokenToClipboard);
document
  .getElementById("resetNowBtn")
  ?.addEventListener("click", goToResetPassword);
document
  .getElementById("closeTokenBtn")
  ?.addEventListener("click", closeDialog);

document
  .querySelector("#adminContactDialog .btn-close")
  ?.addEventListener("click", closeDialog);
document
...
document
  .getElementById("updateGradesForm")
  ?.addEventListener("submit", function (e) {
    e.preventDefault();
